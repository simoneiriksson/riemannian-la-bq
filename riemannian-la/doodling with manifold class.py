...
class MyModel3(torch.nn.Module):
    def __init__(self, num_params, fn, prior_log_sigma):
        super().__init__()
        self.num_params = num_params
        self.fc1 = nn.Linear(num_params, 1)
...
fig, axs = distributions_plot(posterior_samples, MAP, my_covariance_matrix)
fig.show()
...
class Manifold():
    def __init__(self, model, task_type, device="cpu", regularization=0.1, noise=0.1):
    ...
    def fit(self, epochs=1, lr=0.1, optimizer=None, verbose=False, print_every_epoch=100):
        ...
        for epoch in range(epochs):
            ...
            for batch_no, (x, y) in enumerate(train_loader):
                ...
                # the following line only works for regression
                loss_from_pred = self.criterion(pred, y)*N/x.shape[0] / (2 * target_log_sigma.exp()**2)
                # for classification, we need to use the likelihood:
                # loss_from_pred = -likelihood_given_outputs(pred).log_prob(y).sum()
                # Is that correct?
                loss = loss_from_pred + param_norm
                loss.backward()
                optimizer.step()
        ...
        grad_, self.hess_ = LA_approximation(self.model, dataloader=self.train_loader, task_type=self.task_type, prior_sigma=prior_log_sigma.exp(), target_sigma=target_log_sigma.exp())
        #self.MAP_covariance = torch.inverse(hess_)
    ...
    def ode_fun(self, t, state):
        ...
        theta = state_tensor[0,self.MAP.shape[0]]
...
mymanifold = Manifold(model, task_type="regression")
