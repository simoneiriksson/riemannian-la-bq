...
from models import MyModel, mini, PyroModel, PyroModel_subset, PyroModel2
...
class MyModel3(torch.nn.Module):
    def __init__(self, num_params, fn, prior_log_sigma):
        super().__init__()
        self.num_params = num_params
        self.fc1 = nn.Linear(num_params, 2)
...
from homebrew_distr import CustomNormal
...
print(f"{la.mean = }")
print(f"{la.H*la._H_factor + la.prior_precision_diag = }")
...
################
# Laplace approximation using my own LA function
...
cov = hess_['weights']['weights'].inverse().detach()
fig, axs = distributions_plot(posterior_samples, MAP.detach(), cov)
fig.show()