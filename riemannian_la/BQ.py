...
class BayesianQuadrature():
    def __init__(self, Rsampler: Riemann_sampler, evaluation_model, measure="gaussian_rescaled", 
        ...
        self.model_output = self.y.unsqueeze(0).detach().numpy()
        self.thetas = self.theta_init.unsqueeze(0).detach().numpy()
    ...
    # GPy takes X and Y values at initialization. Those will be overwritten later when the emukit model is initialized.
        ...
        self.gpy_model = GPy.models.GPRegression(X=self.vs, 
                                                 Y=self.model_output, 
        ...
        self.emukit_method = VanillaBayesianQuadrature(base_gp=self.emukit_model, X=self.vs, Y=self.model_output)
    ...
    def step(self):
        ...
        self.y_new, theta = self.integrand(v_new_t)
        self.thetas = np.append(self.thetas, theta.numpy())
        self.vs = np.append(self.vs, v_new, axis=0)
        self.model_output = np.append(self.model_output, self.y_new.unsqueeze(0).detach().numpy(), axis=0)
        self.emukit_method.set_data(self.vs, self.model_output)
    ...
    def lebesgue_integrand_rescale(self, z):
        ...
        return self.functional_fwd(theta) * z_dist.log_prob(z).exp(), theta

    def gaussian_integrand(self, v):
        num_ts = self.num_timesteps
        res, ts = R_sampler.expmap_torchdiffeq(R_sampler.mean, v, num_ts=num_ts)
        theta = res[-1,:R_sampler.num_params].T
        return self.functional_fwd(theta), theta
    ...
    def plot2d(self):
        if self.Rsampler.num_params != 2:
            ...
            axes[0].scatter(self.vs[:,0], self.vs[:,1], c=self.model_output[:,0] * z_dist.log_prob(torch.tensor(self.vs)).exp().detach().numpy(), cmap="viridis")
            ...
            axes[1].scatter(self.vs[:,0], self.vs[:,1], c=self.model_output, cmap="viridis")
            ...
            axes[4].scatter(self.vs[:,0], self.vs[:,1], c="k", marker="x")
            #axes[3].scatter(xys_plt[:,0], xys_plt[:,1], c=np.sqrt(var_plot), cmap="viridis")
