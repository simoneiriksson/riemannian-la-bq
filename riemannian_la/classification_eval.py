...
def eval_classification_loss(predictions, true_y, logger = None):
    ...
    ECE_metric = torchmetrics.classification.MulticlassCalibrationError(num_classes=N_classes, n_bins=15)
    acc_metric = torchmetrics.classification.Accuracy(task="multiclass", num_classes=N_classes)
    auroc_metric = torchmetrics.classification.AUROC(task="multiclass", num_classes=N_classes)
